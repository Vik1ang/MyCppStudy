...
#define TEST_STRING(expect, json)\
	do {\
		...
		lept_init(&v);\
		EXPECT_EQ_INT(leptjson::PARSE_OK, leptjson::lept_parse(&v, json));\
		EXPECT_EQ_INT(leptjson::LEPT_STRING, leptjson::lept_get_type(&v));\
		EXPECT_EQ_STRING(expect, leptjson::lept_get_string(&v), leptjson::lept_get_string_length(&v));\
		lept_free(&v);\
	} while(0)
...
static void test_parse_array()
{
	...
	lept_init(&v);
	EXPECT_EQ_INT(leptjson::PARSE_OK, lept_parse(&v, "[ \"abc\", [ 1, 2 ] , 3 ]"));
...
static void test_parse_object() {
	...
	lept_init(&v);
	EXPECT_EQ_INT(leptjson::PARSE_OK, lept_parse(&v, " { } "));
	EXPECT_EQ_INT(leptjson::LEPT_OBJECT, lept_get_type(&v));
	...
	EXPECT_EQ_INT(leptjson::LEPT_NULL, lept_get_type(lept_get_object_value(&v, 0)));
	EXPECT_EQ_STRING("f", lept_get_object_key(&v, 1), lept_get_object_key_length(&v, 1));
	EXPECT_EQ_INT(leptjson::LEPT_FALSE, lept_get_type(lept_get_object_value(&v, 1)));
	EXPECT_EQ_STRING("t", lept_get_object_key(&v, 2), lept_get_object_key_length(&v, 2));
	EXPECT_EQ_INT(leptjson::LEPT_TRUE, lept_get_type(lept_get_object_value(&v, 2)));
	EXPECT_EQ_STRING("i", lept_get_object_key(&v, 3), lept_get_object_key_length(&v, 3));
	EXPECT_EQ_INT(leptjson::LEPT_NUMBER, lept_get_type(lept_get_object_value(&v, 3)));
	...
	EXPECT_EQ_INT(leptjson::LEPT_STRING, lept_get_type(lept_get_object_value(&v, 4)));
	...
	EXPECT_EQ_INT(leptjson::LEPT_ARRAY, lept_get_type(lept_get_object_value(&v, 5)));
	EXPECT_EQ_SIZE_T(3, lept_get_array_size(lept_get_object_value(&v, 5)));
	for (i = 0; i < 3; i++) {
		leptjson::lept_value* e = lept_get_array_element(lept_get_object_value(&v, 5), i);
		EXPECT_EQ_INT(leptjson::LEPT_NUMBER, lept_get_type(e));
		EXPECT_EQ_DOUBLE(i + 1.0, lept_get_number(e));
	}
	EXPECT_EQ_STRING("o", lept_get_object_key(&v, 6), lept_get_object_key_length(&v, 6));
	{
		leptjson::lept_value* o = lept_get_object_value(&v, 6);
		EXPECT_EQ_INT(leptjson::LEPT_OBJECT, lept_get_type(o));
		for (i = 0; i < 3; i++) {
			leptjson::lept_value* ov = lept_get_object_value(o, i);
			EXPECT_TRUE('1' + i == lept_get_object_key(o, i)[0]);
			EXPECT_EQ_SIZE_T(1, lept_get_object_key_length(o, i));
			EXPECT_EQ_INT(leptjson::LEPT_NUMBER, lept_get_type(ov));
...
{
	TEST_ERROR(leptjson::PARSE_MISS_QUOTATION_MARK, "\"");
	TEST_ERROR(leptjson::PARSE_MISS_QUOTATION_MARK, "\"abc");
}

static void test_parse_invalid_string_escape()
{
#if 1
	TEST_ERROR(leptjson::PARSE_INVALID_STRING_ESCAPE, "\"\\v\"");
	TEST_ERROR(leptjson::PARSE_INVALID_STRING_ESCAPE, "\"\\'\"");
	TEST_ERROR(leptjson::PARSE_INVALID_STRING_ESCAPE, "\"\\0\"");
	TEST_ERROR(leptjson::PARSE_INVALID_STRING_ESCAPE, "\"\\x12\"");
#endif
}
...
static void test_parse_miss_comma_or_square_bracket() {
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_SQUARE_BRACKET, "[1");
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_SQUARE_BRACKET, "[1}");
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_SQUARE_BRACKET, "[1 2");
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_SQUARE_BRACKET, "[[]");
}

static void test_parse_miss_key() {
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{1:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{true:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{false:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{null:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{[]:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{{}:1,");
	TEST_ERROR(leptjson::PARSE_MISS_KEY, "{\"a\":1,");
}

static void test_parse_miss_colon() {
	TEST_ERROR(leptjson::PARSE_MISS_COLON, "{\"a\"}");
	TEST_ERROR(leptjson::PARSE_MISS_COLON, "{\"a\",\"b\"}");
}

static void test_parse_miss_comma_or_curly_bracket() {
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_CURLY_BRACKET, "{\"a\":1");
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_CURLY_BRACKET, "{\"a\":1]");
	TEST_ERROR(leptjson::PARSE_MISS_COMMA_OR_CURLY_BRACKET, "{\"a\":1 \"b\"");
...
static void test_access_null()
{
	leptjson::lept_value v;
	lept_init(&v);
	leptjson::lept_set_string(&v, "a", 1);
	lept_set_null(&v);
	EXPECT_EQ_INT(leptjson::LEPT_NULL, leptjson::lept_get_type(&v));
	leptjson::lept_free(&v);
}
...
static void test_access_string()
{
	leptjson::lept_value v;
	lept_init(&v);
	leptjson::lept_set_string(&v, "", 0);
	EXPECT_EQ_STRING("", leptjson::lept_get_string(&v), lept_get_string_length(&v));
	leptjson::lept_set_string(&v, "Hello", 5);
	EXPECT_EQ_STRING("Hello", leptjson::lept_get_string(&v), lept_get_string_length(&v));
	leptjson::lept_free(&v);
}
